...
int main(int argv, char *argc[])
{
    ...
    arma::mat xfen(3,ensemble),q,r,hxen(observe,ensemble),xaen(3,ensemble),yo(3,1);
    double xa[3];
    ...
    for(int i=0; i<steps; i++)
    {
        for(int en=0; en<ensemble; en++)
        {
            ...
            hxen(0,en)=lz[en].result()[1];//+random.gaussian(0,1));
            hxen(1,en)=lz[en].result()[2];//+random.gaussian(0,1));
            //hxen(2,en)=lz[en].result()[2];//+random.gaussian(0,1));
        }
        if((i+1)%obs_delta==0) //has observation
        {
            mat yo=zeros<mat>(observe,1);
            yo(0)=xobs(1,i);
            yo(1)=xobs(2,i);
