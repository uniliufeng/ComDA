...
#pragma omp parallel for
        ...
        for(int en=0; en<ensemble; en++)
        {
            ...
            if ((i+1) % 100 == 0)
            {
                x00[0]=xassim(0,i)*random.uniform(0.8,1.2);
                x00[1]=xassim(1,i)*random.uniform(0.8,1.2);
                x00[2]=xassim(2,i)*random.uniform(0.8,1.2);
            }
            else
            {
                x00[0]=particle_filter.getXaEn()(0,en);//lz[en].result()[0];
                x00[1]=particle_filter.getXaEn()(1,en);//lz[en].result()[1];
                x00[2]=particle_filter.getXaEn()(2,en);//lz[en].result()[2];
            }
            lz[en].parameter(x00);
        }
