/**
\file GHF with Lorenz Model example
*/
#include "lorenz.h"
#include "random.hpp"
...
int main(int argv, char *argc[])
{
    ...
    //compute NSE
    /*
    mat avg_q=zeros<mat>(3,1);
    mat nse1=zeros<mat>(3,1);
    mat nse2=zeros<mat>(3,1);
    int count=0;
    for(int i=0;i<steps;i++)
	{
		if ((i+1) % obs_delta == 0)
	...
	for(int i=0;i<steps;i++)
	{
		if ((i+1) % obs_delta == 0)
		{
			...
			nse1(2)+=pow(xassim(2,i)-xobs(2,i),2);
			nse2(0)+=pow(xobs(0,i)-avg_q(0),2);
			nse2(1)+=pow(xobs(1,i)-avg_q(1),2);
			nse2(2)+=pow(xobs(2,i)-avg_q(2),2);
		}
	}
	mat nse=1-nse1/nse2;
	std::cout<<"#NSE "<<nse(0)<<" "<<nse(1)<<" "<<nse(2)<<std::endl;
	*/
    return 0;
}
