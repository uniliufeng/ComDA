...
Cldas::Cldas(int argc,char** argv)
{
    //ctor
    MPI_Init(&argc,&argv);
...
void Cldas::run()
{
    ...
    for (int istep=1; istep<=m_config.total_steps; istep++)
    {
        ...
        MPI_Status status;
        MPI_Request request;
        int istart,iend,idelta;
        idelta=m_config.total_patches/(numprocs-1);
        if (myrank>0)
        {
            //********************************************************
            //step 3 simulated patch by patch
            //********************************************************
            istart=(myrank-1)*idelta;
            iend=(myrank*idelta>m_config.total_patches)?m_config.total_patches:myrank*idelta;
#pragma omp parallel for
            for (int ipatch=istart; ipatch<iend; ipatch++) run(ipatch);
            MPI_Send(fvar_mean[istart],(iend-istart)*NFVAR,MPI_DOUBLE,0,istep*2,MPI_COMM_WORLD);
            //MPI_Wait(&request,&status);
            MPI_Send(flux_mean[istart],(iend-istart)*OUTFLUX,MPI_DOUBLE,0,istep*2+1,MPI_COMM_WORLD);
            //MPI_Wait(&request,&status);
        }
        else
		{
			...
			{
				istart=(i-1)*idelta;
				iend=(i*idelta>m_config.total_patches)?m_config.total_patches:i*idelta;
				MPI_Recv(fvar_mean[istart],(iend-istart)*NFVAR,MPI_DOUBLE,i,istep*2,MPI_COMM_WORLD,&status);
				//MPI_Wait(&request,&status);
				MPI_Recv(flux_mean[istart],(iend-istart)*OUTFLUX,MPI_DOUBLE,i,istep*2+1,MPI_COMM_WORLD,&status);
				//MPI_Wait(&request,&status);
				std::cout<<"procs "<<i<<" received."<<std::endl;
			}
            //*************************************************************************
            // step 4    output simulated variable
            //*************************************************************************
            cout<<t_time<<", OutPut results..."<<endl;
            ...
            //*************************************************************************
            cout<<"caculation is finished in current time:"<<t_time<<endl;
            //*************************************************************************
		}
    }
