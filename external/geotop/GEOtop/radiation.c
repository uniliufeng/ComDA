...
/******************************************************************************************************************************************/
/*
 * state: Which formula for incoming longwave radiation
 * pvap: saturated vapor pressure
 * Output: eps,eps_max,eps_min
 */
...
/******************************************************************************************************************************************/
/******************************************************************************************************************************************/
/*calculate sun height and sun azimuth in rad*/
void sun(double JD, double *alpha, double *direction, double *E0, double latitude, double longitude, double standard_time){

	/**longitude of the standard time meridian in rad
	 * Gamma:the fractional year in radians =2.0*Pi*JD/365.;
	 * Et:Correction for sideral day (rad)
	 * Delta(Solar Declination):Solar declination (d, rad) is the angle between the equatorial plane and
	 * 			the straight line joining the centers of the earth and the sun.
	 * */
	...
	/**correction to actual sun-earth distance at any specific day in the year,The distance between the earth and the sun varies
	 * every day and is a minimum on January 4 (perihelion) and a maximum on July 6 (aphelion).*/
	*E0=1.00011+0.034221*cos(Gamma)+0.00128*sin(Gamma)+0.000719*cos(2*Gamma)+0.000077*sin(2*Gamma);
	...
	/**Solar altitude angle in rad [rad]
	 * h:solar hour of the day,恒星时
	 * omega: rate of earth's turn, equal to 0.2618 rad h^-1,地球旋转角速度
	 */
...
/******************************************************************************************************************************************/
/******************************************************************************************************************************************/
/* Calcuate the absorved short wave radiation in each snow layer
 * Output: frac[SnowLayerNumber]
 */
...
/******************************************************************************************************************************************/
/******************************************************************************************************************************************/
/**calculate the tau in cloudness condition from Pressure,Temp etc
 * Input:
 * Output: *tau, *sa
 */
void find_tau_cloud_station(long i, METEO *met, PAR *par, double alpha, double E0, double sky, double A, double *tau, double *sa){

	/**tau_atm: global clear sky atmospheric transmittance
	 * kd     : fraction of diffused short wave radiation of the total = d/(d+beam)
	 * kd :Ratio diffuse to global radiation - Erbs et al.(1982)
	 * */
