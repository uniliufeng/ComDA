...
/** read the filenames from the __geotop.inpts;
 * 	the filenames order in files are the same to the order of the __geotop.init (first column)
 */
...
if(par->point_sim!=1){ /* distributed simulation */	
	/** read topography,soil,land use, Drainage direction, network,slope,aspect files */
...
/** Nr=total Rows;Nc=Total Cols;Nl=Total Layers;NoV=No Value*/
times->time=0.0;
Nr=top->Z0->nrh;
Nc=top->Z0->nch;
NoV=UV->V->co[2];
...
//METEO DATA
//meteo parameters __meteo.txt
f=t_fopen(join_strings(files->co[fmet]+1,textfile),"r");
index=read_index(f,PRINT);
/** meteo station metadata, stored in METEO_STATION struct */
...
/** Initial met->st(METEO_STATION) with IT->met (doublematrix)*/
init_meteo_stations(IT->met, met->st);

//meteo data
/** METEO*/
met->column=alloc_long2(IT->met->nrh);/** Row:the number of stations,Cols:the columns of the meteo input file
										  用来保存每一个站点的驱动变量在输入文件中的列数*/
met->data=(double ***)malloc(IT->met->nrh*sizeof(double**));
met->horizon=(double ***)malloc(IT->met->nrh*sizeof(double**));
met->var=alloc2(met->st->Z->nh,nmet);	//meteo variables for the current instant 13
...
if(strcmp(files->co[fLRs]+1 , MISSING_FILE) != 0){
	//s stands for string
...
for(i=1;i<=par->n_landuses;i++){
	...
	if(strcmp(files->co[fvegpar]+1 , MISSING_FILE) != 0){
	...
	for(j=1;j<=IT->land_classes->nch;j++){
		...
		if(i==(long)IT->land_classes->co[1][j]){
			...
			//find root fraction 根据根深、层数，各层厚度，计算各层的根分布*/
...
//在河网图中搜索高程最高的河道网格，然后依次向下游标记，记录行列行，直到流域出口点；接着在剩下的河道网格中依次搜索，寻找未标记的
//海拔最高的河道网格，并依次向下游标记，直到汇入主干河道；依次类推，知道标记记录所有河道网格的行列号。
if(i>1){
	i=0;
	do{
		//寻找河道中海拔最高的点的行列数
...
if(existing_file(files->co[flu]+1)>0){
	//landuse (reading or write default values)
	land->LC=read_map(1, files->co[flu]+1, top->Z0, UV);
	//Check borders 将四周两行数据设置为空值
...
if(existing_file(files->co[fdd]+1)>0){
	...
	/** Give a channel network file */
	par->channel_network = 1;
	
	/** slope along Drainage Direction for each pixel*/
...
/**
 * par	:	the pointer to a PAR struct
 * times:	a pointer to a TIMES struct
 * program:	a pointer to a char string specifying the parameter file's name
 * ext	:	a pointer to a char string specifying the extention of the input file
 */
	DOUBLEVECTOR *V;
	double Dt_output;
	long index;
