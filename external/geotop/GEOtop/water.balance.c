...
void water_balance(ALLDATA *adt){
	...
	/* P: Psi from the solution of Richards*/
	DOUBLETENSOR *P;
	
	short out, sux;
	long r, c, l, iter, sy;

	/* tb:	inner iterate beginning time for each sub interval/子迭代开始积分时间
	 * te:	inner iterate ending time for each sub interval/子迭代结束积分时间
	 * Dt:	integrating time interval for sub iteration/子积分步长
	 * Dt0:	Outer integrating time interval/总积分步长
	 * n:	时间细化系数
	 * */
		...
		do{
			...
			do{
				...
				if(sux==0 && Dt>adt->P->DtminWb){
					...
					n*=adt->P->nredDtWb;/*par->nredDtWb=2;*/
					out=0;
...
/* Dt:	Time step for solution
 * P:	Psi for each soil layer
 * Nl:	Layer numbers of soil
 * loss:
 * */
short Richards(double Dt, DOUBLETENSOR *P, double *loss, long *iter, ALLDATA *adt){
	...
	/* n:	total nodes in the soil system
	 * */
	long i, l, r, c, m, cont, cont2, iter_tot=0, n=(Nl+1)*adt->P->total_pixel;	
	static long cum_iter;/*accumulated iterated numbers*/
	short out, out2;	
	int sux;
...
int find_matrix_K(DOUBLEVECTOR *Lx, ALLDATA *adt, DOUBLEVECTOR *H, double Dt){
	 ...
	 * kn:	hydraulic conductivity next layer
	 * dz:
	 * dzn:	thickness next layer
	 * */
