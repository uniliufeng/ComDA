...

{
	...
	/*Radiation variables
	 ...
	 sa = Fmax ( a , sin(alpha) ), where a is 0.01 to 0.1
	...
	/* Upward shortwave radiation above the vegetation*/
	double SWupabove_v_vis, SWupabove_v_nir, SWup_above_v;
	/**if SWdata=2, have both beam and diffused shortwave data;if SWdata=1, only have global short wave data*/
	short SWdata;
	...
	double snowD, glacD=0.0, Prain_on_snow, fsnow, fsnownew, fsnowcan, Mr_snow, Er_snow, Sr_snow, Mr_glac, Er_glac, Sr_glac;
	 ...
	 /*Precipitation variables
	  ...
	  Prain, Psnow: total precipitation at the surface in the Below the Canopy
	...
	/** when have short wave radiation and sun hight >0,tau_cloud_yes=1,else tau_cloud_yes=0*/
	 ...
	 * Tpoint
	 * Precpoint
	 * zmeas_T corrected measure height of temperature according to snow depth
	 * zmeas_u corrected measure height of wind speed according to snow depth
	 * */
	...
	/**SHADOW & CLOUDINESS calculate sun height and sun azimuth in rad*/
	...
	if(met->column[met->nstsrad-1][iSWb]!=-1 && met->column[met->nstsrad-1][iSWd]!=-1){
		if(met->var[met->nstsrad-1][met->column[met->nstsrad-1][iSWb]]!=NoV && met->var[met->nstsrad-1][met->column[met->nstsrad-1][iSWd]]!=NoV){
			SWdata=2;/**have both beam and diffuse short wave radiance */
		}else{
			SWdata=0;
		}
	}else if(met->column[met->nstsrad-1][iSW]!=-1){
		if(met->var[met->nstsrad-1][met->column[met->nstsrad-1][iSW]]!=NoV){
			SWdata=1;/** have global short wave radiance*/
		}else{
			SWdata=0;
		}
	}else{
	...
	// whether have tau data or cloud cover data
	if( met->column[met->nstcloud-1][itauC]!=-1 ){
		...
		if(tau_cloud_av!=NoV){
			if(tau_cloud_av_yes>1) tau_cloud_av_yes=1.0;
	...
	for(r=1;r<=Nr;r++){
		for(c=1;c<=Nc;c++){
			...
			if(land->LC->co[r][c]!=NoV){//if the pixel is not a novalue
				...
				//Determine the snow depth in mm and snow layer number
				snowD=DEPTH(r, c, snow->lnum, snow->Dzl);			
				ns=snow->lnum->co[r][c];
			
				//vegetation parameters,calculate canopy fraction according to snow depth, LAI
...
/************************************************************ƒ******************************************************************************/
/*
 * zmu: wind measuring height
 * zmT: Temperature measuring height
 * z0s: surface roughness length
 * d0s: surface displacement height
 * z0v: roughness length for veg after considering snow depth
 * d0v:	displacement height for veg after considering snow depth
 * hveg:	canopy height after considering snow depth
 * v:	windspeed
 * Ta:	air Temp
 * LR:	Lapse Rate
 * eps:	emissivity of the surface
 * decaycoeff0: Decay coefficient of the eddy diffusivity profile in the canopy
 * Wcrn: liquid precipitation intercepted by vegetation
 * SWin: incoming shortwave radiation
 * SWv: net shortwave absorbed by vegetation
 * LW:	net longwave at the surface
 * H:	net longwave at the surface
 * E:	evaporation at the surface
 * LWv:	longwave absorbed by vegetation
 * Hv: sensible heat flux at the canopy
 * LEv: latent heat flux at the canopy
 ...
 * Eg1: evaporation at the surface in the vegetated fraction
 * Aerodyamic resistances, h=heat v=water vapor, c=canopy for water vapour, b=boundary layer canopy i.e. canopy for heat,
   uc=undercanopy=surface below the canopy---rh,rv,rb,rc,r_uc
 * Locc = below canopy Monin-Obukhov length
 * LWupabove_v: shortwave above vegetation directed upwards
*/
void PointEnergyBalance(long r, long c, ENERGY *egy, LAND *land, SOIL *sl, PAR *par, long ns, long ng, double zmu, double zmT, 
	...
	/*Tg surface skin temperature used to compute the surface energy fluxes
	 *
	 *
     */
	...
	/*Tv0: canopy temperature at the beginning of the time step*/
...
/******************************************************************************************************************************************/
/******************************************************************************************************************************************/
/*	EnergyFluxes above the surface
 *
 *	Calculate the canopy temperature(Tv), air temperature in canopy(Ts,Qs).
 * 	Calculate the sensible and latent heat flux at the veg free and under the vegetation
 *  Longwave radiation absorbed by veg, surface and up-welling long-wave radiation above veg
 *
 *  Output: H,E,Hg0,Hg1,Eg0,Eg1,dE_dT,dH_dT
 *  		LW,LWup_above_v
 *  		Tv,Ts,Qs
 *
 * H:	net longwave at the surface
...
void EnergyFluxes(double Tg, long r, long c, long n, double Tg0, double Qg0, double Tv0, double zmu, double zmT, double z0s, 
	...
	*dE_dT=0.0;	
	...
	/*LW:	net longwave radiation absorbed by surface*/
	*LW=0.0; 
	*LWup_above_v=0.0;
	...
	*LEv=0.0; 
	...
	/* Hg0: sensible heat from surface at veg free part
	 * Hg1:	sensible heat from surface under veg part
	 * Eg0:
	 * Eg1:
	 * */
	*Etrans=0.0; 																																													
