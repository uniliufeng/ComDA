...
static char vcid[] = "$Id: initialize_model_state.c,v 5.8.2.18 2009/09/28 22:12:48 vicadmin Exp $";

int Vic::initialize_model_state(dist_prcp_struct    *prcp,
			   dmy_struct           dmy,
	...
	for( band = 0; band < options.SNOW_BAND; band++ ) {
	  ...
	  if ( soil_con->AreaFract[band] > 0. ) {
		...
		cell[dry][veg][band].layer[lidx].ice = ice[veg][band][lidx];
	      ...
	      if ( !( options.LAKES && veg == lake_con.lake_idx ) && options.FROZEN_SOIL && soil_con->FS_ACTIVE ) {
...
int Vic::update_thermal_nodes(dist_prcp_struct    *prcp,
			  int                  Nveg,
			  int                  Nnodes,
			  int                  Ndist,
			  soil_con_struct     *soil_con,
