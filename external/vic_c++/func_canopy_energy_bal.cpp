...
double Vic::func_canopy_energy_bal(Vic* pvic,double Tfoliage, va_list ap)
/*********************************************************************
 ...
 ********************************************************************/
{
  ...
  *NetLongOver  = LongOverIn - (*LongOverOut);
  ...
  if ( IntSnow > 0 ) {
    ...
    if (pvic->options.AERO_RESIST_CANSNOW == AR_COMBO || pvic->options.AERO_RESIST_CANSNOW == AR_406 || pvic->options.AERO_RESIST_CANSNOW == AR_406_LS || pvic->options.AERO_RESIST_CANSNOW == AR_406_FULL)
      Ra_used[1] *= 10.;

    /** Calculate the vapor mass flux between intercepted snow in 
	the canopy and the surrounding air mass **/
  
    EsSnow = pvic->svp(Tfoliage); 
    
    /* Apply stability correction to aerodynamic resistance */
    if (pvic->options.AERO_RESIST_CANSNOW == AR_COMBO || pvic->options.AERO_RESIST_CANSNOW == AR_410) {
      if (Wind[1] > 0.0) {
        Ra_used[1] /= pvic->StabilityCorrection(ref_height[1], displacement[1], Tfoliage, 
    ...
    *Evap = 0;
    ...
    if (pvic->options.AERO_RESIST_CANSNOW == AR_406)
      Ra_used[1] /= 10;
  }
  else {

    if (pvic->options.AERO_RESIST_CANSNOW == AR_406_FULL || pvic->options.AERO_RESIST_CANSNOW == AR_410 || pvic->options.AERO_RESIST_CANSNOW == AR_COMBO) {
      Ra_used[0] = Ra[0];
      Ra_used[1] = Ra[1];
    }
    else {
    ...
    prec[DRY] = 0;
    *Evap = pvic->canopy_evap(layer_wet, layer_dry, veg_var_wet, veg_var_dry, FALSE, 
