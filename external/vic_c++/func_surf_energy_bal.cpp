...
static char vcid[] = "$Id: func_surf_energy_bal.c,v 5.8.2.14 2009/09/20 02:32:07 vicadmin Exp $";

double Vic::func_surf_energy_bal(Vic* pvic, double Ts, va_list ap)
/**********************************************************************
...
/*   double             kappa_snow; */
  ...
  ***************/
  ...
  if(pvic->options.GRND_FLUX) {
    ...
    ***************************************************************/
    ...
    if ( pvic->options.QUICK_FLUX ) {
      /**************************************************************
        Use Liang et al. 1999 Equations to Calculate Ground Heat 
	Flux
      **************************************************************/
      *T1 = pvic->estimate_T1(TMean, T1_old, T2, D1, D2, kappa1, kappa2, Cs1, 
			Cs2, dp, delta_t);
    
      /*****************************************************
	Compute the Ground Heat Flux from the Top Soil Layer
      *****************************************************/
      if (pvic->options.GRND_FLUX_TYPE == GF_406) {
    ...
    *************************************************************/
      ...
      /* IMPLICIT Solution */
      if(pvic->options.IMPLICIT) {
	Error = pvic->solve_T_profile_implicit(Tnew_node, T_node, Zsum_node, kappa_node, Cs_node, 
	...
	if(FIRST_SOLN[1]){
      ...
      if(!pvic->options.IMPLICIT || Error == 1) {
	if(pvic->options.IMPLICIT)
	  FIRST_SOLN[0] = TRUE;
#if QUICK_FS
	Error = pvic->solve_T_profile(Tnew_node, T_node, Tnew_fbflag, Tnew_fbcount, Zsum_node, kappa_node, Cs_node, 
    ...
    ******************************************************/
    if((FS_ACTIVE && pvic->options.FROZEN_SOIL) && (TMean+ *T1)/2.<0.) {
      ice = moist - pvic->maximum_unfrozen_water((TMean+ *T1)/2.,
#if EXCESS_ICE
    ...
    if (pvic->options.GRND_FLUX_TYPE == GF_FULL) {
    ...
    if (FS_ACTIVE && pvic->options.FROZEN_SOIL) {
      if (pvic->options.GRND_FLUX_TYPE == GF_FULL) {
  ...
  if ( wind[UnderStory] > 0.0 && overstory && SNOWING )
    ra_under = ra[UnderStory] 
      / pvic->StabilityCorrection(ref_height[UnderStory], 0.f, TMean, Tair, 
			    wind[UnderStory], roughness[UnderStory]);
  else if ( wind[UnderStory] > 0.0 )
    ra_under = ra[UnderStory] 
      / pvic->StabilityCorrection(ref_height[UnderStory], displacement[UnderStory], 
  ...
  /*************************************************
    ...
    winter crop planted).
  *************************************************/
  if ( VEG && !SNOWING && pvic->veg_lib[veg_class].LAI[month-1] > 0 ) {
    Evap = pvic->canopy_evap(layer_wet, layer_dry, veg_var_wet, veg_var_dry, TRUE, 
    ...
    Evap = pvic->arno_evap(layer_wet, layer_dry, NetBareRad, Tair, vpd, 
  ...
  *latent_heat_sub = 0.;
  ...
  if (INCLUDE_SNOW) {
    ...
    pvic->latent_heat_from_snow(atmos_density, vp, Le, atmos_pressure, 
