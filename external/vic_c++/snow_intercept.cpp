...
static char vcid[] = "$Id: snow_intercept.c,v 5.7.2.7 2009/09/20 02:32:07 vicadmin Exp $";
...
*****************************************************************************/
int Vic::snow_intercept(double  AirDens,
		    double  Dt, 
		    double  EactAir, 		    
		    double  F,  
		    double  LAI, 
...
double Vic::solve_canopy_energy_bal(double Tfoliage, ...)
{
  va_list  ap;
  double   Qnet;

  va_start(ap, Tfoliage);
  Qnet = func_canopy_energy_bal(this,Tfoliage, ap);
  va_end(ap);

  return Qnet;
}

double Vic::error_calc_canopy_energy_bal(double Tfoliage, ...)
{
  va_list  ap;
  double   Qnet;
...
double Vic::error_print_canopy_energy_bal(double Tfoliage, va_list ap)
{  
