...
int Vic::solve_lake(double             snowfall,
	       double             rainfall,
	       double             tair, 
	       double             wind, 
	       double             vp, 
...
void Vic::latsens (double Tsurf, double Tcutk, double hice, double tair, double wind,
...
void Vic::alblake (double  Tcutoff, 
	      double  Tair, 
	      double *snowalbedo, 
	      double *albs, 
	      float  *albi, 
...
void Vic::colavg (double *finaltemp, double *T,double *Ti,float lakeprv,double *density, int numnod, double dz, double surfdz)
  {
...
double Vic::calc_density(double ts)
  {
...
void Vic::eddy (int freezeflag, double wind, double *T, double *water_density, 
...
void Vic::iceform (double *qfusion, 
	      double *T, 
	      double  Tcutoff, 
	      double  fracprv,
	      double *areaadd, 
...
int Vic::lakeice (double *tempi, double Tcutoff, double sw_ice,
...
float Vic::lkdrag (float Tsurf, double Tair, double wind, double roughness, double Z1)
{
...
void Vic::temp_area(double sw_visible, double sw_nir, double surface_force, 
...
void Vic::tracer_mixer (double *T, 
                   int    *mixdepth,
		   int     freezeflag,
                   double *surface,
                   int     numnod, 
...
void Vic::energycalc (double *finaltemp, double *sumjoule, int numnod, double dz, double surfdz, double *surface, double *cp, double *density)
{
...
int Vic::water_balance (lake_var_struct *lake, lake_con_struct lake_con, int dt, dist_prcp_struct *prcp,
...
**********************************************************************/
{
   ...
   *    wetland.  Outgoing runoff and baseflow are in meters over lake surface.
   **********************************************************************/
  ...
  lake->soil.runoff = lake->runoff_out;
  ...
  if (newfraction < 1.0) { // wetland exists at end of time step
    for (dist=0; dist<Ndist; dist++) {
      advect_soil_veg_storage(lakefrac, max_newfraction, newfraction, delta_moist, &soil_con, &veg_con, &(cell[dist][iveg][band]), &(veg_var[dist][iveg][band])); 
      rescale_soil_veg_fluxes((1-lakefrac), (1-newfraction), &(cell[dist][iveg][band]), &(veg_var[dist][iveg][band])); 
    }
  ...
  if (newfraction > 0.0) { // lake exists at end of time step
    if (lakefrac > 0.0) { // lake existed at beginning of time step
      rescale_soil_veg_fluxes(lakefrac, newfraction, &(lake->soil), NULL); 
      rescale_snow_energy_fluxes(lakefrac, newfraction, &(lake->snow), &(lake->energy)); 
      rescale_lake_fluxes(lakefrac, newfraction, lake); 
    }
      ...
      initialize_lake(lake, lake_con, &soil_con, energy[iveg][band].T[0]);
...
void Vic::advect_soil_veg_storage(double lakefrac,
                             double max_newfraction,
                             double newfraction,
                             double *delta_moist,
                             soil_con_struct  *soil_con,
...
void Vic::rescale_soil_veg_fluxes(double oldfrac,
                             double newfrac,
                             cell_data_struct *cell,
                             veg_var_struct   *veg_var)
{
...
void Vic::advect_snow_storage(double lakefrac,
                         double max_newfraction,
                         double newfraction,
                         snow_data_struct  *snow)
{
...
void Vic::rescale_snow_energy_fluxes(double oldfrac,
                                double newfrac,
                                snow_data_struct  *snow,
                                energy_bal_struct *energy)
{
...
void Vic::rescale_lake_fluxes(double oldfrac,
                         double newfrac,
                         lake_var_struct *lake)
{
