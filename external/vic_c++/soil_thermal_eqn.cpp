...
double Vic::soil_thermal_eqn(Vic* pvic, double T, va_list ap) {
  ...
  ******************************************************************/
  ...
  if(T<0.) {
#if QUICK_FS
    ice = moist - pvic->maximum_unfrozen_water_quick(T, max_moist,
					       ufwc_table);
#else
#if EXCESS_ICE
    ice = moist - pvic->maximum_unfrozen_water(T,porosity,effective_porosity,max_moist,bubble,expt);
#else
    ice = moist - pvic->maximum_unfrozen_water(T,max_moist,bubble,expt);
#endif
#endif
    if(ice<0.) ice=0.;
