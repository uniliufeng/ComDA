...
*****************************************************************************/
double Vic::SnowPackEnergyBalance(Vic* pvic, double TSurf, va_list ap)
{
  ...
  if (Wind > 0.0)
    Ra_used[0] = Ra / pvic->StabilityCorrection(Z, 0.f, TMean, Tair, Wind, Z0[2]);
  else
  ...
  *SensibleHeat = AirDens * Cp * (Tair - TMean) / Ra_used[0];
  ...
  pvic->latent_heat_from_snow(AirDens, EactAir, Lv, Press, Ra_used[0], TMean, Vpd,
