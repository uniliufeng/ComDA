...
*****************************************************************************/
double Vic::IceEnergyBalance(Vic* pvic, double TSurf, va_list ap)
{
...
/*   TMean = 0.5 * (OldTSurf + TSurf); */
  ...
  if (Wind > 0.0)
    *Ra_used = Ra / pvic->StabilityCorrection(Z, 0.f, TMean, Tair, Wind, Z0);
...
/*     EsSnow *= 1.0 + .00972 * TMean + .000042 * pow((double)TMean,(double)2.0); */
  ...
  pvic->latent_heat_from_snow(AirDens, EactAir, Lv, Press, Ra, TMean, Vpd,
