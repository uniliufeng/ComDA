...
static char vcid[] = "$Id: initialize_atmos.c,v 5.9.2.7 2009/10/13 21:35:10 vicadmin Exp $";

void Vic::initialize_atmos(atmos_data_struct        *atmos,
                      dmy_struct               *dmy,
		      FILE                    **infile,
                      double                    theta_l,
                      double                    theta_s,
...
printf("Precipitaion");
  ...
  ******************************************************/
  //printf("Shortwave, vp, air temp, pressure, and density\n");

  if(!(param_set.TYPE[TMAX].SUPPLIED && param_set.TYPE[TMIN].SUPPLIED)) {
    rec = 0;
    printf("global_param.nrecs %d£¬stepspday1 %d\n,NF %d,NR %d,step %d",global_param.nrecs,stepspday,NF,NR,step);
  ...
  **********************************************************************/
  //  printf("Calculate the subdaily and daily temperature based on tmax and tmin\n");
