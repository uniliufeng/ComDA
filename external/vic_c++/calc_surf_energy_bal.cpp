...
static char vcid[] = "$Id: calc_surf_energy_bal.c,v 5.9.2.13 2009/09/20 02:32:07 vicadmin Exp $";

double Vic::calc_surf_energy_bal(double             Le,
			    double             LongUnderIn,
...
double Vic::solve_surf_energy_bal(double Tsurf, ...) {
  ...
  va_start(ap, Tsurf);
  error = func_surf_energy_bal(this,Tsurf, ap);
  va_end(ap);
...
double Vic::error_calc_surf_energy_bal(double Tsurf, ...) {
...
double Vic::error_print_surf_energy_bal(double Ts, va_list ap) {
/**********************************************************************
  Modifications:
